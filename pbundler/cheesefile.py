...
class Cheese(object):
    ...
    def exact_version(self):
        ...
        if not self.is_exact_version():
            raise Exception("Cheese %s didn't have an exact version (%s)" % (self.name, self.version_req))
...
class CheesefileContext(object):
    ...
    def cheese(self, name, version=None, platform=None, path=None):
        self.groups[self.current_group].append(Cheese(name, version, platform, path))
